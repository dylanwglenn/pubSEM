...
var targetPadding float32 = 10

func CalculateModel(m *Model, gtx layout.Context) {
	...
	for _, n := range m.Nodes {
		...
		case OBSERVED:
			...
			for e := 0; e < 4; e++ {
				...
				angleMap := make(map[*Connection]float64, len(connections))
	...
	for _, c := range m.Connections {
		...
		switch {
		case c.Type != CIRCULAR:
			if c.Origin.Class == LATENT {
...
func AssignToEdges(c *Connection, nodes []*Node) {
	switch {
	case c.Origin.Class == OBSERVED && c.Destination.Class == LATENT:
	...
	case c.Origin.Class == LATENT && c.Destination.Class == OBSERVED:
	...
	case c.Origin.Class == OBSERVED && c.Destination.Class == OBSERVED:
...
func GetBestEdge(candidateEdges []int, destNode, originNode *Node, originEdgePoint utils.LocalPos, nodes []*Node) int {
	...
	for i, edge := range candidateEdges {
		...
		intersections[i] = checkIntersection(originEdgePoint, destEdgePoint, nodes, []*Node{destNode, originNode})
	}
...
func InvertAngle(c *Connection) float64 {
	var angle float64
	switch c.Type {
	case STRAIGHT:
...
func SubdivideNodeEdge(n *Node, edge, numPoints int) []utils.LocalPos {
	if n.Class != OBSERVED {
		return nil
	}
...
func checkIntersection(originPos, destPos utils.LocalPos, nodes, exclusion []*Node) bool {
	for _, n := range nodes {
